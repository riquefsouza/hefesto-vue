...
export default {
    setup() {
        ...
        const onExport = () => {
            admUserService.value.report(reportParamForm.value).then(() => {
                toast.add({ severity: 'info', summary: 'User Exported', detail: 'User Exported', life: 3000 });
            });
        }
        ...
        const onDelete = () => {
            admUserService.value.delete(admUser.value.id).then(() => {
