...
export default {
    setup() {
        ...
        const onExport = () => {
            admPageService.value.report(reportParamForm.value).then(() => {
                toast.add({ severity: 'info', summary: 'Page Exported', detail: 'Page Exported', life: 3000 });
            });
        }
        ...
        const onDelete = () => {
            admPageService.value.delete(admPage.value.id).then(() => {
